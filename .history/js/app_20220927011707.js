...
document.body.addEventListener("click", function (e) {
  ...
  function removeItemCart(e, priority = 1) {
    if (e.target.matches(".fa-trash-can") && priority === 1) {
      ...
      totalPriceFnc();
      cartQuantityFnc();
    } else if (e.target.matches(".decr-qty") && priority === -1) {
      const product = e.target.parentNode.parentNode.parentNode.parentNode;
      const idProduct = product.dataset.id;
      product.remove();
      const indexRemove = listCart.findIndex((item) => item.id === idProduct);
      listCart.splice(indexRemove, 1);
  ...
  if (e.target.matches(".decr-qty")) {
    ...
    const indexIncr = listCart.findIndex((item) => item.id === idProductQty);
    ...
    resetData();
    totalPriceFnc();
  } else if (e.target.matches(".incr-qty")) {
...
const cartEmpty = document.querySelector(".cart-top--empty");
function totalPriceFnc() {
  let pricevalue = 0;
  [...listCart].forEach((item) => {
    const priceItem = item.priceCar;
    const qtyItem = item.no;
    pricevalue += priceItem * qtyItem;
...
// show quatity of cart icon
const cartQty = document.querySelector(".cart-quantity");
function cartQuantityFnc() {
  const lenCart = listCart.length;
  cartQty.textContent = lenCart;
}
cartQuantityFnc();

...
window.addEventListener("scroll", function (e) {
  console.log(achievedArea.offsetTop);
  console.log(window.scrollY);
  if (window.pageYOffset > 4100 && window.pageYOffset < 4200) {
    // achievedItem.forEach((item) => debounce(counter(item), 2000));
  } else return;
});

function counter(el) {
  const numberEl = el.querySelector(".achieved-number");
  const to = parseInt(numberEl.textContent);
  let count = 0;
  const step = 100;
  const countUp = to / step;
  setInterval(() => {
    count += countUp;
    if (count > to) {
      clearInterval();

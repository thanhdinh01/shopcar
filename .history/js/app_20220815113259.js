...
document.addEventListener("click", function (e) {
  // selectItem.forEach(item=>if (
  //   // !e.target.matches(".dropdown-select") &&
  ...
  selectItem.forEach((item) => {
    if (!item.contains(e.target)) {
      ...
      // e.target.lastElementChild.classList.remove("fa-angle-up");
    }
  });
});
...
// document.addEventListener("click", function (e) {
//   console.log(e.target);
//   if (!dropdownSelect.forEach((item) => item.contains(e.target))) {
//     dropdownList.forEach((item) => item.classList.remove("show"));
//     console.log("oke");
//   }
// });

...
function renderItemCart({ img, nameCar, priceCar, no }) {
  const template = `
  <div class="cart-item">
...
document.body.addEventListener("click", function (e) {
  if (e.target.matches(".button-book")) {
    ...
    if (JSON.parse(localStorage.getItem("itemsCart")) === null) {
      ...
      // renderItemCart(itemCart);
      // localItems.map((data) => {
      // console.log(localItems[index].id);
      // el1,el2,el3
      // if (itemCart.id == data.id) {
      // console.log(el.id);
      // itemCart.no = data.no + 1;
  ...
  if (e.target.matches(".fa-trash-can")) {
    ...
    product.remove();
    // [...listCart].forEach(
    //   (item) => console.log(item.img)
    // item.findIndex((el) => el.img === product.img)
    // );
    // console.log(index);
  }
});

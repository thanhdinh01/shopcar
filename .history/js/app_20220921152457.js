...
document.body.addEventListener("click", function (e) {
  if (e.target.matches(".button-book")) {
    ...
    const nameCar =
      e.target.parentNode.nextElementSibling.children[0].children[0].children[0]
        .textContent;
    const pricecar =
      e.target.parentNode.nextElementSibling.children[1].children[1].children[1]
        .children[1].textContent;
    renderItemCart(img, nameCar, pricecar);
  }
});

function renderItemCart(img, title, price) {
  const template = `
  <div class="cart-item">
  ...
  `;
  cartTop.insertAdjacentElement("beforeend", template);
}

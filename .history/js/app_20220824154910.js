...
const achievedNums = document.querySelectorAll(".achieved-number");
let timers = [];
let started = false;
...
const counter = (durration) => {
  ...
  timers[index] = setInterval(() => {
    if (item.innerHTML >= goal) {
    }
    item.dataset.current += countUp;
...
window.addEventListener("scroll", function (e) {
  if (
    ...
    if (started) return;
    achievedNums.forEach((item) => counter(30));
    started = true;
  } else {
    resetCounter();
    started = false;
